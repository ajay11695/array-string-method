...
function countAllPeople(){
  let countpeople=got.houses.reduce((acc,cv)=>{
...
function nameWithS(){
  return got.houses.reduce((acc,cv)=>{
    acc=acc.concat(cv.people.map(p=>p.name).filter(n=>n.includes("s")|| n.includes("S")))
    return acc
  },[])
}
...
function nameWithA(){
  return got.houses.reduce((acc,cv)=>{
    acc=acc.concat(cv.people.map(p=>p.name).filter(n=>n.includes("a")|| n.includes("A")))
    return acc
  },[])
}
...
function surnameWithS(){
  return got.houses.reduce((acc,cv)=>{
    acc=acc.concat(cv.people.map(p=>p.name).filter(n=>n.split(" ")[1].includes("S")))
    return acc
  },[])
}
...
function surnameWithA(){
  return got.houses.reduce((acc,cv)=>{
    acc=acc.concat(cv.people.map(p=>p.name).filter(n=>n.split(" ")[1].includes("A")))
    return acc
  },[])
}
